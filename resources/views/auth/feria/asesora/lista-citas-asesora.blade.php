...
@section('titulo')
    <title>BolsaTrabajo | Citas</title>
@endsection
    ...
    <div class="content-wrapper">
        <section class="content-header d-flex justify-content-between align-items-center">          
            ...
            <div>
                <a href="javascript:void(0)" class="btn-m btn-secondary-m" onclick="window.location.reload();">
                    <i class="fa fa-refresh"></i> Refrescar | Listado de Citas
                </a>
            </div>
        </section>
        @if(session('success'))
        ...
        <div class="content-header bg-search" style="">
            <p for="actividad_eco_filter_id" class="m-0 label-primary text-dark-green">Filtro
                por Rango de Fecha de Inicio de las Citas</p>
            <div class="row">
                @php
        ...
        <section class="content-header">
            <div class="row">
                <div class="col-12">
                    <table id="tableAsesora" width="100%" class='table dataTables_wrapper container-fluid dt-bootstrap4 no-foote'>
                        <thead>
                            <th>Nombre de la Asesora</th>
                            <th>Nombre del Alumno</th>
                            <th>Celular del Alumno</th>
                            <th>Motivo</th>
                            <th>Fecha/Hora</th>
                            <th>Estado</th>
                            <th>Acciones</th>
                        </thead>
                        <tbody id="tablaAsesoras">
                        </tbody>
                    </table>
                </div>
            </div>
...
@section('scripts')
    ...
    <script>
        const url = "{{ route('auth.ferias.filtrar-citas-asesoras') }}";
        const baseUrlFinalizado = "{{ route('auth.ferias.finalizar-citas-asesora', ['id' => ':id']) }}";
    </script>
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    <script type="text/javascript" src="{{ asset('auth/js/feria/filtro-asesora.js') }}"></script>
    @if(session('success'))
